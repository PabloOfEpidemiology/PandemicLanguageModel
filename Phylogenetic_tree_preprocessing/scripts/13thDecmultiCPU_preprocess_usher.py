...
def extract_features(
    ...
    columns_file_out = os.path.join("simulation_results", f"{args.current_pango_loss}_columns_{max_num_clades}.pkl")
    ...
    if not args.skip_features:
        ...
        proto, tree = load_proto(pruned_tree_filename)
        nuc_mutations_by_clade = load_mutation_tree(proto, tree)[0]
        assert nuc_mutations_by_clade
        ...
        features_file_out = os.path.join("results", f"{args.current_pango_loss}_features_{max_num_clades}.pt")
...
def main(args):
    ...
    # Store these directly in args - December 2024 modification
    args.refined_proto = proto
    args.refined_tree = tree
