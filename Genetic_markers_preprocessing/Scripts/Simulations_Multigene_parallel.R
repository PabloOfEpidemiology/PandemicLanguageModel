...
output_prefix_base <- if (GLOBAL) {
  paste0("Multigene_Global_", mutation_type)
} else {
  paste0("Multigene_Countries_", mutation_type)
}
...
output_dir <- "Simulations/Multigene"
...
if (mutation_type == "RNA") {
  data.count_N.name <- file.path(input_dir, "Multigene_S_ORF3a_ORF7a_ORF8_N/filtered_multigene_aa_count_table.txt")
...
if (mutation_type == "RNA") {
  FG_mut.info.name <- file.path(input_dir, "Multigene_S_ORF3a_ORF7a_ORF8_N/multigene_full.nextclade.mut_long.tsv")
} else if (mutation_type %in% c("NSP", "ORF")) {
  FG_mut.info.name <- file.path(input_dir, "Multigene_S_ORF3a_ORF7a_ORF8_N/multigene_full.nextclade.mut_long.tsv")
} else {
  stop("Unknown mutation_type")
