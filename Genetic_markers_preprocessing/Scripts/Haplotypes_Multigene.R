...
# Initialize mutation_type as NULL
mutation_type <- NULL
...
# Parse mutation type flags
if ("--NSP" %in% args) {
  if (!is.null(mutation_type)) {
    stop("Only one of --NSP or --ORF can be specified.")
  }
  mutation_type <- "NSP"
}
...
# Ensure that a mutation_type has been specified
if (is.null(mutation_type)) {  # <-- Modified
  stop("You must specify one of --NSP or --ORF")
}
...
# Define the output prefix based on the GLOBAL switch, mutation_type, and count.mut_threshold  # <-- Modified
output_prefix <- if (GLOBAL) {
  paste0(mutation_type, "_Global_", count.mut_threshold)
} else {
  paste0(mutation_type, "_Countries_", count.mut_threshold)
}
...
input_dir_Multigene <- if (CLUSTER) {
  "/data/zool-paleovirology/ball6168/PandemicLanguageModel/Genetic_markers_preprocessing/Input_data/FG/Multigene_S_ORF3a_ORF7a_ORF8_N/"
} else {
  "test_Inputs/"
}
...
# Set file names based on mutation_type ----  # <-- Added
if (mutation_type %in% c("NSP", "ORF")) {
  data.count_N.name <- file.path(input_dir_Multigene, "filtered_multigene_aa_count_table.txt")
  Multigene_mut.info.name <- file.path(input_dir_Multigene, "multigene_full.nextclade.mut_long.tsv")
} else {
...
output_subdir <- paste0("Multigene_", output_prefix)
...
# Function to get cache file path with mutation_type and GLOBAL flags  # <-- Modified
get_cache_file <- function(threshold, GLOBAL, mutation_type) {  # <-- Modified
  global_str <- ifelse(GLOBAL, "Global", "Countries")  # <-- Modified (removed underscore to match desired prefix)
  file.path(cache_dir, paste0("Multigene_", mutation_type, "_", global_str, "_", threshold, ".rds"))  # <-- Modified
}

cache_file <- get_cache_file(count.mut_threshold, GLOBAL, mutation_type)  # <-- Modified
...
if (is_cache_valid(cache_file)) {
  ...
  required_vars <- c("Multigene_metadata.filtered.interest", "Multigene_count.hap.mat", "Multigene_count.df.hap2", 
                     "Multigene_count.df.hap2.min", "Multigene_hap_Id.ref", "Multigene_count.df.mut", 
                     "country.interest.df", "bin.size", "generation_time",
                     "country.interest.v", "Multigene_mut.info.name", "Multigene_hap.info.df","Multigene_count.df.hap")
...
if (process_from_scratch) {
  ...
  metadata.name <- file.path(input_dir_Multigene, "Multigene_full.nextclade.tsv")
  ...
  # Merge with metadata
  metadata.merged <- metadata %>% inner_join(data.count_N, by = "seqName")
  ...
  Multigene_mut.info <- fread(Multigene_mut.info.name, header = TRUE, sep = "\t", quote = "", check.names = TRUE)
  print("Loaded mutation information. First few rows:")
  print(head(Multigene_mut.info))
  
  
  if (mutation_type == "NSP") {
    # For NSP, use the column 'mut_NSP'
    Multigene_mut.info <- Multigene_mut.info %>% dplyr::select(seqName, mut = mut_NSP)  # <-- Modified variable name
  } else if (mutation_type == "ORF") {
    # For ORF, use the column 'mut_ORF'
    Multigene_mut.info <- Multigene_mut.info %>% dplyr::select(seqName, mut = mut_ORF)  # <-- Modified variable name
  } else {
    stop("Unknown mutation_type")  # <-- Added
  }
  
  # Join with metadata.merged
  Multigene_mut.info <- Multigene_mut.info %>% right_join(metadata.merged %>% dplyr::select(seqName), by = "seqName")
  print("Joined mutation info with filtered metadata. Rows in merged data:")
  print(nrow(Multigene_mut.info))
  
  # Count mutations and filter based on count
  Multigene_count.df.mut <- Multigene_mut.info %>% group_by(mut) %>% summarize(count.mut = n())
  Multigene_count.df.mut <- Multigene_count.df.mut %>% arrange(desc(count.mut)) %>% filter(count.mut > count.mut_threshold)
  print(paste("Mutations with counts >", count.mut_threshold, ":"))
  print(Multigene_count.df.mut)
  
  # Further refine the mutation information
  Multigene_mut.info <- Multigene_mut.info %>% inner_join(Multigene_count.df.mut %>% dplyr::select(mut), by = "mut")
  Multigene_mut.info <- Multigene_mut.info %>% unique()
  Multigene_mut.info <- Multigene_mut.info %>% mutate(mut = as.factor(mut))
  print("Refined mutation information. Unique mutations count:")
  print(length(unique(Multigene_mut.info$mut)))
  
  # Summarize haplotype information
  
  # Convert mut.info to a data.table
  Multigene_mut.info.dt <- as.data.table(Multigene_mut.info)
  # Summarize haplotype information with the correct delimiter
  Multigene_mut.info.sum.dt <- Multigene_mut.info.dt[, .(haplotype = paste(mut, collapse = ", ")), by = seqName]
  # Convert back to tibble if needed
  Multigene_mut.info.sum <- as_tibble(Multigene_mut.info.sum.dt)
  print("Haplotype information summarized using data.table with correct delimiter.")
  
  Multigene_mut.info.sum <- Multigene_mut.info.sum %>% mutate(hap_Id = paste("hap_", as.character(as.numeric(as.factor(haplotype))), sep=""))
  Multigene_mut.info.sum <- Multigene_mut.info.sum %>% inner_join(metadata.merged %>% dplyr::select(seqName, country), by="seqName")
  print("Haplotype information summarized and joined with metadata.")
  print(head(Multigene_mut.info.sum))
  
  # Unique haplotype information
  Multigene_hap.info.df <- Multigene_mut.info.sum %>% ungroup() %>% dplyr::select(hap_Id, haplotype) %>% unique()
  print("Unique haplotypes information:")
  print(Multigene_hap.info.df)
  
  # Group by haplotype and country, filter by count >= 20
  Multigene_count.df.hap <- Multigene_mut.info.sum %>% group_by(hap_Id, country) %>% summarize(count.hap = n()) %>% arrange(desc(count.hap))
  Multigene_count.df.hap <- Multigene_count.df.hap %>% filter(count.hap >= 20)
  print("Haplotypes with counts >= 20:")
  print(Multigene_count.df.hap)
  
  # Filter for haplotypes in count.df.hap
  Multigene_mut.info.sum <- Multigene_mut.info.sum %>% filter(hap_Id %in% as.character(Multigene_count.df.hap$hap_Id))
  
  # Merge haplotypes back with metadata
  metadata.merged <- metadata.merged %>% inner_join(Multigene_mut.info.sum %>% dplyr::select(seqName, hap_Id), by = "seqName")
  metadata.merged <- metadata.merged %>% distinct(seqName, .keep_all = TRUE)
  ...
  Multigene_metadata.filtered.interest <- metadata.merged %>%
  ...
  Multigene_metadata.filtered.interest <- Multigene_metadata.filtered.interest %>% filter(!is.na(date.bin.num))
  print("Filtered metadata with date bins. Rows remaining:")
  print(nrow(Multigene_metadata.filtered.interest))
  
  # Create count matrix for haplotypes
  Multigene_count.hap.mat <- Multigene_metadata.filtered.interest %>%
  ...
  print(Multigene_count.hap.mat)
  
  # Gather data for haplotype count comparisons
  Multigene_count.df.hap2 <- Multigene_count.hap.mat %>% gather(key = country, value = count, -hap_Id)
  Multigene_count.df.hap2.min <- Multigene_count.df.hap2 %>% group_by(hap_Id) %>% summarize(count.min = min(count)) %>% arrange(desc(count.min))
  Multigene_hap_Id.ref <- Multigene_count.df.hap2.min$hap_Id[1]
  print(paste("Reference haplotype ID:", Multigene_hap_Id.ref))
  print("Count matrix for reference haplotype:")
  print(Multigene_count.hap.mat %>% filter(hap_Id == Multigene_hap_Id.ref))
  
  # Save preprocessed data to cache
  print(paste("Saving preprocessed data to cache for count.mut_threshold =", count.mut_threshold))
  save(list = c("Multigene_metadata.filtered.interest", "Multigene_count.hap.mat", "Multigene_count.df.hap2", 
                "Multigene_count.df.hap2.min", "Multigene_hap_Id.ref", "Multigene_count.df.mut", 
                "country.interest.df", "bin.size", "generation_time",
                "country.interest.v", "Multigene_mut.info.name", "Multigene_hap.info.df","Multigene_count.df.hap"), 
...
Multigene_data.coef.df <- data.frame()
# Convert to data.table for faster processing
setDT(Multigene_metadata.filtered.interest)

# Extract the desired columns: country, date.bin.num, hap_Id
Multigene_metadata_to_save <- Multigene_metadata.filtered.interest[, .(country, date.bin.num, hap_Id)]
...
results <- foreach(country.interest = country.interest.df$country,
                   ...
                   .export = c("country.interest.df", "Multigene_metadata.filtered.interest", "Multigene_hap_Id.ref", "bin.size", "generation_time")) %dopar% {
                     tryCatch({
                       start_time <- Sys.time()
                       print(paste("[", format(start_time, "%Y-%m-%d %H:%M:%S"), "]", "Starting processing for country:", country.interest))
                       
                       # Filter data for the current country
                       Multigene_metadata.filtered.interest.country <- Multigene_metadata.filtered.interest[country == country.interest]
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                                   "Filtered data size for", country.interest, ":", nrow(Multigene_metadata.filtered.interest.country)))
                       
                       if(nrow(Multigene_metadata.filtered.interest.country) == 0){
                         print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                       ...
                       Multigene_metadata.filtered.interest.bin <- Multigene_metadata.filtered.interest.country[, .(count = .N), by = .(date.bin.num, hap_Id)]
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                       ...
                       hap.count.df <- Multigene_metadata.filtered.interest.bin[, .(total = sum(count)), by = hap_Id][total >= 20][order(-total)]
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                       ...
                       Multigene_metadata.filtered.interest.bin.spread <- dcast(Multigene_metadata.filtered.interest.bin[hap_Id %in% hap.count.df$hap_Id],
                                                                         date.bin.num ~ hap_Id,
                                                                         value.var = "count",
                                                                         fill = 0)
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                                   "Data spread completed with dimensions:", paste(dim(Multigene_metadata.filtered.interest.bin.spread), collapse = "x")))
                       
                       # Create X matrix for the model
                       X <- cbind(1, Multigene_metadata.filtered.interest.bin.spread$date.bin.num)
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                                   "X matrix created with dimensions:", paste(dim(X), collapse = "x")))
                       
                       # Create Y matrix for the model
                       Y <- as.matrix(Multigene_metadata.filtered.interest.bin.spread[, -1, with = FALSE])
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                       ...
                       # Order hap_Id
                       hap_Id.order.v <- c(Multigene_hap_Id.ref, setdiff(hap.count.df$hap_Id, Multigene_hap_Id.ref))
                       Y_cols <- colnames(Y)
                       ...
                       max.date <- max(Multigene_metadata.filtered.interest.country$date.bin.num)
                       date.v <- X[,2] / max.date
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                                   "Max date:", max.date))
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                       ...
                       fit.coef <- rbindlist(list(data.table(hap_Id = Multigene_hap_Id.ref, relative_Re = 1), fit.coef))
                       ...
                       rm(Multigene_metadata.filtered.interest.country,
                          Multigene_metadata.filtered.interest.bin,
                          Multigene_metadata.filtered.interest.bin.spread,
                          X, Y)
                       gc()
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
...
Multigene_metadata.filtered.interest.country <- Multigene_metadata.filtered.interest[country %in% country.interest.df$country]
print("Filtered data for countries of interest")

# Create bin counts
Multigene_metadata.filtered.interest.bin <- Multigene_metadata.filtered.interest.country[, .(count = .N), by = .(country, date.bin.num, hap_Id)]
print("Created binned counts")

# Create hap_country column
Multigene_metadata.filtered.interest.bin[, hap_country := paste(hap_Id, country, sep="_")]
print("Created hap_country column")

Multigene_metadata.filtered.interest.bin <- Multigene_metadata.filtered.interest.bin[, .SD[sum(count) >= 20], by = hap_country]
print("Filtered out hap_country combinations with fewer than 20 counts")

# Get min and max dates
min_date <- min(Multigene_metadata.filtered.interest.bin$date.bin.num)
max_date <- max(Multigene_metadata.filtered.interest.bin$date.bin.num)
...
unique_hap_country <- unique(Multigene_metadata.filtered.interest.bin$hap_country)
print(paste("Number of unique hap_country combinations:", length(unique_hap_country)))

# Reshape the data from long to wide format
hap_counts_wide <- dcast(Multigene_metadata.filtered.interest.bin,
...
Multigene_data.coef.df <- as.data.frame(results)
...
Multigene_mut.info <- Multigene_mut.info %>% right_join(Multigene_metadata.filtered.interest %>% dplyr::select(seqName,hap_Id), by = "seqName")
...
Multigene_mut.info.dt <- as.data.table(Multigene_mut.info)
Multigene_mut.info.sum.all_S <- Multigene_mut.info.dt[, .(haplotype.all = toString(mut)), by = seqName]
print("Haplotype information summarized.")

Multigene_mut.info.sum.all_S <- Multigene_mut.info.sum.all_S[, hap_Id.all_mut := paste("hap_all_", as.character(as.numeric(as.factor(haplotype.all))), sep="")]
print("Haplotype ID created.")

Multigene_mut.info.sum.all_S <- as_tibble(Multigene_mut.info.sum.all_S)
print("Converting Multigene_mut.info.sum.all_S back to tibble.")

Multigene_metadata.filtered.interest <- Multigene_metadata.filtered.interest %>% inner_join(Multigene_mut.info.sum.all_S %>% dplyr::select(seqName,hap_Id.all_mut),by="seqName")
print("Joined summarized haplotype info with metadata.")

date.quantile.df <- Multigene_metadata.filtered.interest %>% mutate(Collection.date = as.Date(Collection.date)) %>% group_by(hap_Id) %>% summarize(date.first = quantile(Collection.date,0.01,type=1))
...
all_S.major.hap.df <- Multigene_metadata.filtered.interest %>%
...
Multigene_metadata.filtered.interest.representative <- Multigene_metadata.filtered.interest %>% inner_join(all_S.major.hap.df %>% dplyr::select(hap_Id,hap_Id.all_mut),by=c("hap_Id","hap_Id.all_mut")) %>% group_by(hap_Id) %>% sample_n(1)
Multigene_metadata.filtered.interest.representative <- Multigene_metadata.filtered.interest.representative %>% dplyr::select(seqName,clade,Nextclade_pango,hap_Id,Accession.ID)
Multigene_metadata.filtered.interest.representative <- Multigene_metadata.filtered.interest.representative %>% inner_join(Multigene_hap.info.df,by="hap_Id")
print("Representative sequences selected.")

print("Colnames of Multigene_metadata.filtered.interest")
print(colnames(Multigene_metadata.filtered.interest))

print("Colnames of Multigene_metadata.filtered.interest.representative")
print(colnames(Multigene_metadata.filtered.interest.representative))



Multigene_data.coef.df.merged <- Multigene_data.coef.df %>% inner_join(Multigene_metadata.filtered.interest.representative,by="hap_Id")
Multigene_data.coef.df.merged <- Multigene_data.coef.df.merged %>% arrange(desc(relative_Re))
print("Coefficient data merged with representative metadata.")

Multigene_data.coef.df.merged <- Multigene_data.coef.df.merged %>% left_join(date.quantile.df,by="hap_Id")
...
write.table(Multigene_data.coef.df.merged, out.name, col.names=TRUE, row.names=FALSE, sep="\t", quote=FALSE)
print(paste("Output written to", out.name))

print("Saving Multigene_metadata")
# Filter Multigene_metadata_to_save based on the combination of country and hap_Id
Multigene_metadata_to_save_filtered <- Multigene_metadata_to_save %>%
  dplyr::semi_join(Multigene_data.coef.df.merged, by = c("country", "hap_Id"))

# Save the extracted data to a CSV file
fwrite(
  Multigene_metadata_to_save_filtered,
  file.path(output_dir, paste0(output_prefix, "_metadata_filtered_interest.csv"))
)

print("Multigene_metadata saved")
...
Multigene_count.df.hap.all_regions <- Multigene_metadata.filtered.interest %>% group_by(hap_Id) %>% summarize(count.total = n())
print("Count dataframe created.")

Multigene_data.coef.df.merged <- Multigene_data.coef.df.merged %>% inner_join(Multigene_count.df.hap,by=c("hap_Id","country"))
print("Coefficient data merged.")

print("Creating and saving time_vs_fitness_each_country_log.pdf.")
g <- ggplot(Multigene_data.coef.df.merged, aes(x=date.first,y=log(relative_Re), size = count.hap, fill = clade))
...
pdf.name <- file.path(output_dir, paste0(output_prefix, "Multigene_time_vs_fitness_each_country_log.pdf"))
pdf(pdf.name, width = 20, height = 10)
print(g)
dev.off()
print(paste("First plot saved to", pdf.name))

g <- ggplot(Multigene_data.coef.df.merged, aes(x=date.first,y=log(relative_Re+0.1), size = count.hap, fill = clade))
g <- g + geom_point(shape = 21, alpha = 0.7)
g <- g + theme(panel.grid.major = element_blank(),
               panel.grid.minor = element_blank(),
               panel.background = element_blank(),
...
pdf.name <- file.path(output_dir, paste0(output_prefix, "Multigene_time_vs_fitness_each_country_log_with_pseudo.pdf"))
pdf(pdf.name, width = 20, height = 10)
print(g)
dev.off()
print(paste("Second plot saved to", pdf.name))

g <- ggplot(Multigene_data.coef.df.merged, aes(x=date.first,y=relative_Re, size = count.hap, fill = clade))
g <- g + geom_point(shape = 21, alpha = 0.7)
g <- g + theme(panel.grid.major = element_blank(),
               panel.grid.minor = element_blank(),
               panel.background = element_blank(),
...
pdf.name <- file.path(output_dir, paste0(output_prefix, "Multigene_time_vs_fitness_each_country.pdf"))
pdf(pdf.name, width = 20, height = 10)
print(g)
dev.off()
...
haplotype.mut_profile <- Multigene_metadata.filtered.interest.representative %>%
...
haplotype.mut_profile <- haplotype.mut_profile %>%
  dplyr::inner_join(Multigene_metadata.filtered.interest.representative %>%
...
pdf.name <- file.path(output_dir, "Multigene_mut_heatmap.pdf")
...
# Boxplot
ggplot(Multigene_data.coef.df.merged, aes(x = clade, y = relative_Re, fill = clade)) +
...
fitness_vs_mutations <- Multigene_data.coef.df.merged %>%
