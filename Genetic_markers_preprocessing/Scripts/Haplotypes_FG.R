...
# Parse mutation type flags  # <-- Added
if ("--RNA" %in% args) {
...
if (is.null(mutation_type)) {  # <-- Added
  stop("You must specify one of --RNA, --NSP, or --ORF")
}
...
input_dir_FG <- if (CLUSTER) {
  "/data/zool-paleovirology/ball6168/PandemicLanguageModel/Genetic_markers_preprocessing/Input_data/FG/"
} else {
  "test_Inputs/"
}
...
if (mutation_type == "RNA") {
  data.count_N.name <- file.path(input_dir_FG, "filtered_FG_RNA_count_table.txt")
  FG_mut.info.name <- file.path(input_dir_FG, "FG_RNA_full.nextclade.mut_long.tsv")
} else if (mutation_type %in% c("NSP", "ORF")) {
  data.count_N.name <- file.path(input_dir_FG, "filtered_FG_aa_count_table.txt")
  FG_mut.info.name <- file.path(input_dir_FG, "FG_aa_NSP_ORF_full.nextclade.mut_long.tsv")
} else {
...
output_subdir <- paste0("FG_", output_prefix)
...
get_cache_file <- function(threshold, GLOBAL, mutation_type) {  # <-- Modified
  ...
  file.path(cache_dir, paste0("FG_", mutation_type, "_", global_str, "_", threshold, ".rds"))  # <-- Modified
}
...
if (is_cache_valid(cache_file)) {
  ...
  required_vars <- c("FG_metadata.filtered.interest", "FG_count.hap.mat", "FG_count.df.hap2", 
                     "FG_count.df.hap2.min", "FG_hap_Id.ref", "FG_count.df.mut", 
                     "country.interest.df", "bin.size", "generation_time",
                     "country.interest.v", "FG_mut.info.name", "FG_hap.info.df","FG_count.df.hap")
...
if (process_from_scratch) {
  ...
  metadata.name <- file.path(input_dir_FG, "FG_full.nextclade.tsv")
  ...
  FG_mut.info <- fread(FG_mut.info.name, header = TRUE, sep = "\t", quote = "", check.names = TRUE)
  print("Loaded mutation information. First few rows:")
  print(head(FG_mut.info))
  
  
  if (mutation_type == "RNA") {
    # For RNA, mutations are in column 'mut'
    FG_mut.info <- FG_mut.info  # <-- Modified variable name
  } else if (mutation_type == "NSP") {
    # For NSP, use the column 'mut_NSP'
    FG_mut.info <- FG_mut.info %>% dplyr::select(seqName, mut = mut_NSP)  # <-- Modified variable name
  } else if (mutation_type == "ORF") {
    # For ORF, use the column 'mut_ORF'
    FG_mut.info <- FG_mut.info %>% dplyr::select(seqName, mut = mut_ORF)  # <-- Modified variable name
  } else {
    stop("Unknown mutation_type")  # <-- Added
  }
  
  # Join with metadata.merged
  FG_mut.info <- FG_mut.info %>% right_join(metadata.merged %>% dplyr::select(seqName), by = "seqName")
  ...
  FG_count.df.mut <- FG_mut.info %>% group_by(mut) %>% summarize(count.mut = n())
  FG_count.df.mut <- FG_count.df.mut %>% arrange(desc(count.mut)) %>% filter(count.mut > count.mut_threshold)
  print(paste("Mutations with counts >", count.mut_threshold, ":"))
  print(FG_count.df.mut)
  
  # Further refine the mutation information
  FG_mut.info <- FG_mut.info %>% inner_join(FG_count.df.mut %>% dplyr::select(mut), by = "mut")
  ...
  print(length(unique(FG_mut.info$mut)))
  ...
  FG_mut.info.sum <- FG_mut.info.sum %>% mutate(hap_Id = paste("hap_", as.character(as.numeric(as.factor(haplotype))), sep=""))
  FG_mut.info.sum <- FG_mut.info.sum %>% inner_join(metadata.merged %>% dplyr::select(seqName, country), by="seqName")
  print("Haplotype information summarized and joined with metadata.")
  print(head(FG_mut.info.sum))
  
  # Unique haplotype information
  FG_hap.info.df <- FG_mut.info.sum %>% ungroup() %>% dplyr::select(hap_Id, haplotype) %>% unique()
  ...
  metadata.merged <- metadata.merged %>% inner_join(FG_mut.info.sum %>% dplyr::select(seqName, hap_Id), by = "seqName")
  metadata.merged <- metadata.merged %>% distinct(seqName, .keep_all = TRUE)
  ...
  FG_metadata.filtered.interest <- metadata.merged %>%
  ...
  print(nrow(FG_metadata.filtered.interest))
  
  # Create count matrix for haplotypes
  FG_count.hap.mat <- FG_metadata.filtered.interest %>%
  ...
  print(FG_count.hap.mat)
  
  # Gather data for haplotype count comparisons
  FG_count.df.hap2 <- FG_count.hap.mat %>% gather(key = country, value = count, -hap_Id)
  FG_count.df.hap2.min <- FG_count.df.hap2 %>% group_by(hap_Id) %>% summarize(count.min = min(count)) %>% arrange(desc(count.min))
  FG_hap_Id.ref <- FG_count.df.hap2.min$hap_Id[1]
  print(paste("Reference haplotype ID:", FG_hap_Id.ref))
  print("Count matrix for reference haplotype:")
  print(FG_count.hap.mat %>% filter(hap_Id == FG_hap_Id.ref))
  
  # Save preprocessed data to cache
  print(paste("Saving preprocessed data to cache for count.mut_threshold =", count.mut_threshold))
  save(list = c("FG_metadata.filtered.interest", "FG_count.hap.mat", "FG_count.df.hap2", 
                "FG_count.df.hap2.min", "FG_hap_Id.ref", "FG_count.df.mut", 
                "country.interest.df", "bin.size", "generation_time",
                "country.interest.v", "FG_mut.info.name", "FG_hap.info.df","FG_count.df.hap"), 
...
FG_data.coef.df <- data.frame()
# Convert to data.table for faster processing
setDT(FG_metadata.filtered.interest)

# Extract the desired columns: country, date.bin.num, hap_Id
FG_metadata_to_save <- FG_metadata.filtered.interest[, .(country, date.bin.num, hap_Id)]
...
results <- foreach(country.interest = country.interest.df$country,
                   ...
                   .export = c("country.interest.df", "FG_metadata.filtered.interest", "FG_hap_Id.ref", "bin.size", "generation_time")) %dopar% {
                     tryCatch({
                       start_time <- Sys.time()
                       print(paste("[", format(start_time, "%Y-%m-%d %H:%M:%S"), "]", "Starting processing for country:", country.interest))
                       
                       # Filter data for the current country
                       FG_metadata.filtered.interest.country <- FG_metadata.filtered.interest[country == country.interest]
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                                   "Filtered data size for", country.interest, ":", nrow(FG_metadata.filtered.interest.country)))
                       
                       if(nrow(FG_metadata.filtered.interest.country) == 0){
                         print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                       ...
                       FG_metadata.filtered.interest.bin <- FG_metadata.filtered.interest.country[, .(count = .N), by = .(date.bin.num, hap_Id)]
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                       ...
                       hap.count.df <- FG_metadata.filtered.interest.bin[, .(total = sum(count)), by = hap_Id][total >= 20][order(-total)]
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                       ...
                       FG_metadata.filtered.interest.bin.spread <- dcast(FG_metadata.filtered.interest.bin[hap_Id %in% hap.count.df$hap_Id],
                                                                            date.bin.num ~ hap_Id,
                                                                            value.var = "count",
                                                                            fill = 0)
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                                   "Data spread completed with dimensions:", paste(dim(FG_metadata.filtered.interest.bin.spread), collapse = "x")))
                       
                       # Create X matrix for the model
                       X <- cbind(1, FG_metadata.filtered.interest.bin.spread$date.bin.num)
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                                   "X matrix created with dimensions:", paste(dim(X), collapse = "x")))
                       
                       # Create Y matrix for the model
                       Y <- as.matrix(FG_metadata.filtered.interest.bin.spread[, -1, with = FALSE])
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                       ...
                       # Order hap_Id
                       hap_Id.order.v <- c(FG_hap_Id.ref, setdiff(hap.count.df$hap_Id, FG_hap_Id.ref))
                       Y_cols <- colnames(Y)
                       ...
                       max.date <- max(FG_metadata.filtered.interest.country$date.bin.num)
                       date.v <- X[,2] / max.date
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                                   "Max date:", max.date))
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
                       ...
                       fit.coef <- rbindlist(list(data.table(hap_Id = FG_hap_Id.ref, relative_Re = 1), fit.coef))
                       ...
                       rm(FG_metadata.filtered.interest.country,
                          FG_metadata.filtered.interest.bin,
                          FG_metadata.filtered.interest.bin.spread,
                          X, Y)
                       gc()
                       print(paste("[", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "]",
...
FG_metadata.filtered.interest.country <- FG_metadata.filtered.interest[country %in% country.interest.df$country]
print("Filtered data for countries of interest")

# Create bin counts
FG_metadata.filtered.interest.bin <- FG_metadata.filtered.interest.country[, .(count = .N), by = .(country, date.bin.num, hap_Id)]
print("Created binned counts")

# Create hap_country column
FG_metadata.filtered.interest.bin[, hap_country := paste(hap_Id, country, sep="_")]
print("Created hap_country column")

FG_metadata.filtered.interest.bin <- FG_metadata.filtered.interest.bin[, .SD[sum(count) >= 20], by = hap_country]
print("Filtered out hap_country combinations with fewer than 20 counts")

# Get min and max dates
min_date <- min(FG_metadata.filtered.interest.bin$date.bin.num)
max_date <- max(FG_metadata.filtered.interest.bin$date.bin.num)
...
unique_hap_country <- unique(FG_metadata.filtered.interest.bin$hap_country)
print(paste("Number of unique hap_country combinations:", length(unique_hap_country)))

# Reshape the data from long to wide format
hap_counts_wide <- dcast(FG_metadata.filtered.interest.bin,
...
FG_data.coef.df <- as.data.frame(results)
...
FG_mut.info <- FG_mut.info %>% right_join(FG_metadata.filtered.interest %>% dplyr::select(seqName,hap_Id), by = "seqName")
...
FG_metadata.filtered.interest <- FG_metadata.filtered.interest %>% inner_join(FG_mut.info.sum.all_S %>% dplyr::select(seqName,hap_Id.all_mut),by="seqName")
print("Joined summarized haplotype info with metadata.")

date.quantile.df <- FG_metadata.filtered.interest %>% mutate(Collection.date = as.Date(Collection.date)) %>% group_by(hap_Id) %>% summarize(date.first = quantile(Collection.date,0.01,type=1))
...
FG_metadata.filtered.interest.representative <- FG_metadata.filtered.interest %>% inner_join(all_S.major.hap.df %>% dplyr::select(hap_Id,hap_Id.all_mut),by=c("hap_Id","hap_Id.all_mut")) %>% group_by(hap_Id) %>% sample_n(1)
FG_metadata.filtered.interest.representative <- FG_metadata.filtered.interest.representative %>% dplyr::select(seqName,clade,Nextclade_pango,hap_Id,Accession.ID)
FG_metadata.filtered.interest.representative <- FG_metadata.filtered.interest.representative %>% inner_join(FG_hap.info.df,by="hap_Id")
print("Representative sequences selected.")

print("Colnames of FG_metadata.filtered.interest")
print(colnames(FG_metadata.filtered.interest))

print("Colnames of FG_metadata.filtered.interest.representative")
print(colnames(FG_metadata.filtered.interest.representative))



FG_data.coef.df.merged <- FG_data.coef.df %>% inner_join(FG_metadata.filtered.interest.representative,by="hap_Id")
FG_data.coef.df.merged <- FG_data.coef.df.merged %>% arrange(desc(relative_Re))
print("Coefficient data merged with representative metadata.")

FG_data.coef.df.merged <- FG_data.coef.df.merged %>% left_join(date.quantile.df,by="hap_Id")
...
write.table(FG_data.coef.df.merged, out.name, col.names=TRUE, row.names=FALSE, sep="\t", quote=FALSE)
print(paste("Output written to", out.name))

print("Saving FG_metadata")
# Filter FG_metadata_to_save based on the combination of country and hap_Id
FG_metadata_to_save_filtered <- FG_metadata_to_save %>%
  dplyr::semi_join(FG_data.coef.df.merged, by = c("country", "hap_Id"))

# Save the extracted data to a CSV file
fwrite(
  FG_metadata_to_save_filtered,
  file.path(output_dir, paste0(output_prefix, "_metadata_filtered_interest.csv"))
)

print("FG_metadata saved")
...
FG_count.df.hap.all_regions <- FG_metadata.filtered.interest %>% group_by(hap_Id) %>% summarize(count.total = n())
print("Count dataframe created.")

FG_data.coef.df.merged <- FG_data.coef.df.merged %>% inner_join(FG_count.df.hap,by=c("hap_Id","country"))
print("Coefficient data merged.")

print("Creating and saving time_vs_fitness_each_country_log.pdf.")
g <- ggplot(FG_data.coef.df.merged, aes(x=date.first,y=log(relative_Re), size = count.hap, fill = clade))
...
pdf.name <- file.path(output_dir, paste0(output_prefix, "FG_time_vs_fitness_each_country_log.pdf"))
pdf(pdf.name, width = 20, height = 10)
print(g)
dev.off()
print(paste("First plot saved to", pdf.name))

g <- ggplot(FG_data.coef.df.merged, aes(x=date.first,y=log(relative_Re+0.1), size = count.hap, fill = clade))
g <- g + geom_point(shape = 21, alpha = 0.7)
g <- g + theme(panel.grid.major = element_blank(),
               panel.grid.minor = element_blank(),
               panel.background = element_blank(),
...
pdf.name <- file.path(output_dir, paste0(output_prefix, "FG_time_vs_fitness_each_country_log_with_pseudo.pdf"))
pdf(pdf.name, width = 20, height = 10)
print(g)
dev.off()
print(paste("Second plot saved to", pdf.name))

g <- ggplot(FG_data.coef.df.merged, aes(x=date.first,y=relative_Re, size = count.hap, fill = clade))
g <- g + geom_point(shape = 21, alpha = 0.7)
g <- g + theme(panel.grid.major = element_blank(),
               panel.grid.minor = element_blank(),
               panel.background = element_blank(),
...
pdf.name <- file.path(output_dir, paste0(output_prefix, "FG_time_vs_fitness_each_country.pdf"))
pdf(pdf.name, width = 20, height = 10)
print(g)
dev.off()
...
haplotype.mut_profile <- FG_metadata.filtered.interest.representative %>%
...
haplotype.mut_profile <- haplotype.mut_profile %>%
  dplyr::inner_join(FG_metadata.filtered.interest.representative %>%
...
pdf.name <- file.path(output_dir, "FG_mut_heatmap.pdf")
...
# Boxplot
ggplot(FG_data.coef.df.merged, aes(x = clade, y = relative_Re, fill = clade)) +
...
fitness_vs_mutations <- FG_data.coef.df.merged %>%
